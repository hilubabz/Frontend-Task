...
export default function Data() {
  ...
  const columns = [
    ...
    columnHelper.accessor("dateOfExperience", {
      header: "Date of Experience",
      cell: (info) => {
    ...
    columnHelper.accessor((row) => row.social?.facebook ?? "", {
      ...
      header: "Facebook",
      cell: (info) => {
        return <div className="text-blue-500 underline">{info.getValue()}</div>;
      },
    }),
    ...
    columnHelper.display({
      ...
      cell: (info) => {
        return (
          <button
            className="bg-gradient-to-br from-red-500 to-red-800 rounded-xl cursor-pointer text-white px-2 py-1 text-lg"
